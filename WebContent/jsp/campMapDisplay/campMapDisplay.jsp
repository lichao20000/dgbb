...
<script type="text/javascript" >
   ...
   //加入条件筛选加载图表数据  
    ...
    //传入指定条件查询相关数据
        function onChange(){
        	...
        	if(sui.get("tabs").activeIndex==1){
				url="busiSc.do?method=busiScPageAreaC";
	...
	function showAnchor3(url,func,divId,chartId,str,flexPath,type){
		...
		$.ajax({
			...
			dataType:"json",
			url: _url,
			data:{districtBranchCode:code},
			success:function(datas){
				obja = datas;
				var mapName = "营服"+str+"欠费率分布图";
	...
	function setParam(label,url,index,formId,method,chartType,divId,flexPath,chartId){
		...
		var kpid = "";
		if(obja!=null&&obja!=""){
			busiScCode = obja[label].busiSccode;
        ...
        if(haveNext=='Y'){
    		...
    		if(key){
    			//nextPage(code,busiScCode,"${path}/Manager.do?method="+method,index);
    			nextPage2(code,busiScCode,"${path}/shuntJumpAction.do?method=getNextPage",nextType,kpid);
    			}
    		setTimeout(
    				function(){
    					   if(j==-1){
    						   getLineDate(label,code,busiScCode,url,chartType,divId,flexPath,chartId);
    						   chargeParaTable(period, profess, code, busiScCode,"","","bizcs",kpid,formId,client);
    						   }
    			        },300);
            }else{
            	   getLineDate(label,code,busiScCode,url,chartType,divId,flexPath,chartId);
	...
	//ajax取后台折线数据
	function getLineDate(label,districtBranchCode,busiScCode,url,line,divId,flexPath,chartId){	
		var lineName= "";
		if(obja!=null&&obja!=""){
			lineName= obja[label].busiScName;
			}
		var _path = "${path}";
		var _url = _path+url+"&districtBranchCode="+districtBranchCode+"&busiScCode="+busiScCode;
   ...
   function nextPage2(districtBranchCode,busiScCode,url,typeCode,kpid){
		  var url = url+"&branch="+districtBranchCode+"&bizcs="+busiScCode+"&myType="+typeCode+"&kpid="
		  +kpid;
		  url=createUrlWithParam(url);
	  	  window.location.href=url;
		}
  ...
  <body>
   <!-- <div style="background-image:url(${path}/images/u11_original.jpg);widh:100%;font-Family:'微软雅黑',Serif;font-size:14px;padding-bottom:5px;">
    ...
    <div style="background-image:url(${path}/images/u11_original.jpg);width:100%">
      	<div style="background-image:url(${path}/images/u5_original.png);height:29px;width:100%;font-Family:'微软雅黑',Serif; background-repeat: no-repeat;">
    		<div class="tip_title">营服地图展示</div>
    	</div>
    </div> --> 
    <form id="excelForm"
    ...
    <div style=" background-color:#C11403; background-repeat: no-repeat;width:100%;height:3px;margin-top:0.5%;margin-bottom:5px;"></div>
      <div id="tabs" class="sui-tabs" activeIndex="0" style="background-image:url(${path}/images/u11_original.jpg);width:100%;height:678px; background-repeat:repeat-x;padding-left:1%;">
        <div id = "tab1" title="短期欠费率" headerStyle="font-Family:'微软雅黑',Serif;font-size:12px;"  style="margin-bottom:-28px;">
       ...
       <div style="background-image:url(${path}/images/u11_original.jpg);background-repeat: repeat-x;height:49%;width:100%">
        <div id="clients0" class="sui-datagrid" style="width:100%;height:90%;" allowAlternating="true"
            ...
            <div property="columns" headerStyle="font-Family:'微软雅黑',Serif;"> 
				...
				<div field="product_name" name="product_name"  id="product_name" width="80" align="center" headerAlign="center" >产品名称</div> 
        ...
        <div id = "tab2" title="长期欠费率" headerStyle="font-Family:'微软雅黑',Serif;font-size:12px;"   style="margin-left:121px;margin-top:-28px;">
       ...
       <div style="background-image:url(${path}/images/u11_original.jpg);background-repeat: repeat-x;height:49%;;width:100%">
        ...
        <div id = "tab3"  title="总体欠费率" headerStyle="font-Family:'微软雅黑',Serif;font-size:12px;" style="margin-left:242px;margin-top:-30px;">
