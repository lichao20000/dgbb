...
// 可能有bug 需要在调用前设置为true 调用后改为false
var beginValidate_df = true;
...
function checkRmForm_df(thisForm) {
	//附件处理
	if (typeof getIdList != 'undefined' && getIdList instanceof Function) {
		getIdList();
	}
	var checkResult = true;
	rmTempStatusIsAlert_df = false;
	...
	if(validateStr != null) {
		if ( beginValidate_df ) {
			...
			for (var i=0;i<validateTemp.length;i++) {
				...
				if(jQuery(thisInput).attr("class") == "inputselect"){//满足校验下拉控件
					s = replaceSingleQuote_df(jQuery(thisInput).prev().html());
					if(s == "--请选择--"){
...
function notNull_df(s, thisInput, displayInput) {  //不能为空
	...
	var s = Jtrim_df(s);	 
	if ( s.length == 0){
		writeValidateInfo_df_df("请您输入"+inputName+"！ ", displayInput);
		return false;
	}
...
function isMobile_df(s, thisInput) {  //是手机号码：必须以数字开头，除数字外，可含有“-” 
	...
	var patrn=/^(\d{3,4}-)?\d{7,8}$|^(\+86)?1[3|4|5|8][0-9]\d{4,8}$/; 
	if (!patrn.exec(s)) {
...
/**校验小写字母或数字和英文逗号 chrisZ add */
function isNumAndCharacterAndComma_df(s,thisInput){
	var result=s.match(/^[a-z0-9A-Z,]+$/);
    if(result==null){
    writeValidateInfo_df ("请您输入字母或数字或英文逗号！",thisInput);	
    return false;
    }
	hideValidateInfo_df(thisInput);
    return true;
...
function isIP_df(s,thisInput) {  //是IP
	...
	if (!patrn.exec(s)){
		if(!rmTempStatusIsAlert_df){
			writeValidateInfo_df("请输入合法的IP值",thisInput);
			return false;
		}
...
function notNullContactName_df(s, thisInput, displayInput) {  //不能为空
	if(displayInput == null) {
		displayInput = thisInput;
	}
	
	if ( s.length == 0){
		writeValidateInfo_df("请您输入联系人！ ", displayInput);
		return false;
	}  
	var s = Jtrim_df(s);	 
	if ( s.length == 0){
		writeValidateInfo_df_df("请您输入联系人！ ", displayInput);
		return false;
	}
...
function notNullAgentName_df(s, thisInput, displayInput) {  //不能为空
	if(displayInput == null) {
		displayInput = thisInput;
	}
	
	if ( s.length == 0){
		writeValidateInfo_df("请您输入经办人！ ", displayInput);
		return false;
	}  
	var s = Jtrim_df(s);	 
	if ( s.length == 0){
		writeValidateInfo_df_df("请您输入经办人！ ", displayInput);
		return false;
	}
...
function notNullContactDept_df(s, thisInput, displayInput) {  //不能为空
	if(displayInput == null) {
		displayInput = thisInput;
	}
	
	if ( s.length == 0){
		writeValidateInfo_df("请您输入联系人部门！ ", displayInput);
		return false;
	}  
	var s = Jtrim_df(s);	 
	if ( s.length == 0){
		writeValidateInfo_df_df("请您输入联系人部门！ ", displayInput);
		return false;
	}
...
function notNullAgentDept_df(s, thisInput, displayInput) {  //不能为空
	if(displayInput == null) {
		displayInput = thisInput;
	}
	
	if ( s.length == 0){
		writeValidateInfo_df("请您输入经办人部门！ ", displayInput);
		return false;
	}  
	var s = Jtrim_df(s);	 
	if ( s.length == 0){
		writeValidateInfo_df_df("请您输入经办人部门！ ", displayInput);
		return false;
	}
...
function notNullToOrg(s, thisInput, displayInput){  //派发部门不能为空
	if(displayInput == null) {
		displayInput = thisInput;
	}
	
	if ( s.length == 0){
		writeValidateInfo_df("请您选择派发部门！ ", displayInput);
		return false;
	}  
	var s = Jtrim_df(s);	 
	if ( s.length == 0){
		writeValidateInfo_df_df("请您选择派发部门！ ", displayInput);
		return false;
	}
...
//最大长度校验规则
function textareaMaxLength(s, thisInput, length){
	var maxLength = length || 1000;
	
	if (s.length >= maxLength){
		writeValidateInfo_df("长度不能超过1000！ ", thisInput);
		return false;
	}  
