...
// 2009-9-4 10:52  增加电话号码，手机号码精确检测。
/////////////////////////////////////////////////
var reDigit = /^\d+$/
...
var rePhone = /^0(([1-9]\d)|([3-9]\d{2}))\d{8}$/; ////匹配固定电话或小灵通，例如：031185907468或02185907468格式

var reMobile =/^1[3,4,5,8]\d{9}$/;
...
function hasSpecialStr(str, reg){
    if(typeof(reg)=="undefined")
	    return special_str.test(str);
    else
        return reg.test(str);
}

function hasSpecial_email(str, reg){
    if(typeof(reg)=="undefined")
	    return special_email.test(str);
    else
        return reg.test(str);
}
...
//	alert(1);
	...
	//alert(errmsg);
	if(typeof(myText.isnull)!="undefined"&&isnull(myText.value)){
		showMsg(errmsg + "不能为空，或不能只包括空白字符！\n","2",myText);
		myText.focus();
		return false;
	}
	if(!isnull(myText.value)){
//		alert(2.2);
...
//		alert("2.8");
		...
		if (myText.dataType=="email"&& !isEmail(myText.value)){
			showMsg(errmsg + "请输入正确的E-Mail地址（如：admin@hostname.com）！\n", "2", myText);
			myText.focus();
			return false;
		}
//			alert("2.9");
		...
		if (myText.dataType=="phone"&& !isPhone(myText.value)){
			showMsg(errmsg + "请输入正确的电话格式（手机，固定电话或小灵通，\n 例如：13580562222,031185907468或02185907468格式）！\n", "2", myText);
			myText.focus();
			return false;
		}
//			alert("2.10");
...
// 根据dataType参数，判断元素值是否为合法的身份证。
		if (myText.dataType=="iduser"&& !isUserID2(myText.value)){
			showMsg(errmsg + "请输入正确的身份证号码！\n", "2", myText);
			myText.focus();
			return false;
		}
		//特殊字符
		if(myText.dataType=="string" && hasSpecialStr(myText.value)){
...
//////////////////////////////////////////////////
// 函数名：isPhone
//    检查字符串中是否包含不符合要求的字符，符合电话(小灵通，手机等)的字符是数字。
// 参数：
//    str - 检查字符串
// 返回值：
//    true	格式正确
...
{
	return (rePhone.test(str) || reMobile.test(str) );
}
...
//判断指定的字符串是否为一个有效的手机号码
function isMobile(s){
	//var reg = /1[3,5][0-9]\d{8}$/g;
	return reMobile.test(s);
}
...
function  isUserID2(sId){ 
	var  iSum=0 ;
	var  info="" ;
	if(!/^\d{15}|\d{17}[\dX]$/i.test(sId))
		return  false; 
