...
+-----------------------+---------------------------------------+-------------------------------------------------------+
+-----------------------+---------------------------------------+-------------------------------------------------------+
|number					|f_check_numbernotnull(obj)				|数字并不为空			|
+-----------------------+---------------------------------------+-------------------------------------------------------+
|email					|f_check_emailnotnull(obj)				|电子邮件地址并不为空										|
+-----------------------+---------------------------------------+-------------------------------------------------------+
|mobile					|f_check_mobilenotnull(obj)				|移动电话号码并不为空												|
+-----------------------+---------------------------------------+-------------------------------------------------------+
|						|inputnotnull(obj)               		|输入非空校验				|
+-----------------------+---------------------------------------+-------------------------------------------------------+
|						|validationForm()               				|表单提交时验证必填字段为非空				|
+-----------------------+---------------------------------------+-------------------------------------------------------+
*/
...
function validationForm() {
	...
	var str;
	for (; i < tag.length; i++) {
		if (tag[i].onblur != null) {
			str = tag[i].onblur.toString();
			if (str.indexOf("notnull") > -1) {
				if (tag[i].value.length == 0) {
					tag[i].style.border = "2px solid #f4c9cf";
					count = count + 1;
				}
			}
		}
	...
	var j = 0;
	var tag1 = document.getElementsByTagName("select");
	for (; j < tag1.length; j++) {
		if (tag1[j].onblur != null) {
			str = tag1[j].onblur.toString();
			if (str.indexOf("notnull") > -1) {
				if (tag1[j].value.length == 0) {
					tag1[j].parentNode.style.border = "2px solid #f4c9cf";
					count = count + 1;
				}
			}
		}
	...
	var k = 0;
	var tag2 = document.getElementsByTagName("textarea");
	for (; k < tag2.length; k++) {
		if (tag2[k].onblur != null) {
			str = tag2[k].onblur.toString();
			if (str.indexOf("notnull") > -1) {
				if (tag2[k].value.length == 0) {
					tag2[k].parentNode.style.border = "2px solid #f4c9cf";
					count = count + 1;
				}
			}
		}
	...
	var str = str1.replace(/(^\s*)|(\s*$)/g,''); 
		if (str.length==0) {
			if(obj.tagName=="SELECT" || obj.tagName=="TEXTAREA"){
	   		 obj.parentNode.style.border = "2px solid #f4c9cf";
			}else{
	   		 obj.style.border = "2px solid #f4c9cf";
			}
	  		 return false;	
		}else{
			if(obj.tagName=="SELECT" || obj.tagName=="TEXTAREA"){
	   		 obj.parentNode.style.border = "2px none #ffffff";
			}else{
			 obj.style.border = "2px none #ffffff";
			}
	  		 return true;
		}
}
...
//非空
function f_check_numbernotnull(obj)
{   	
	...
	}else if (/^\d+$/.test(Math.abs(obj.value)))
	{
	   obj.style.border = "2px solid #ffffff";
	   return true;
	} 
...
function f_check_email(obj){  
	...
	if(obj.value.length==0){
	  obj.style.border = "2px solid #ffffff";	
	  return true;
	}else if(myReg.test( obj.value )) {
		 obj.style.border = "2px solid #ffffff";
		return true; 
	}
	win_alert("提示信息","请输入合法的电子邮件地址！");
	obj.style.border = "2px solid #f4c9cf";
	return false; 
}

//非空
function f_check_emailnotnull(obj){  
...
function f_check_mobile(obj){   
	...
	//var regu =/(^[1][38][0-9]{9}$)|(^0[1][38][0-9]{9}$)/;
	var re = new RegExp(regu);
	if(obj.value.length==0){
	  obj.style.border = "2px solid #ffffff";	
	  return true;
	...
	win_alert("提示信息", "请输入正确的手机号码！");
	obj.style.border = "2px solid #f4c9cf";
	return false;	
}

//非空
function f_check_mobilenotnull(obj){   
	var regu =/(^((13[0-9])|(15[0-9])|(18[0-9]))\d{8}$)/;
		//var regu =/(^((13[0-9])|(15[^4,\\D])|(18[0,5-9]))\\d{8}$)/;
	var re = new RegExp(regu);
	if(obj.value.length==0){
	  obj.style.border = "2px solid #f4c9cf";	
	  return true;
